...
async function ensureOllamaRunning(){try{await ollamaRequest('/api/tags');return true;}catch{}try{const child=spawn(ollamaExe(),['serve'],{detached:true,stdio:'ignore',windowsHide:true});let spawnFailed=false;child.once('error',()=>{spawnFailed=true;});child.unref();for(let i=0;i<20;i++){if(spawnFailed)break;await new Promise(r=>setTimeout(r,500));try{await ollamaRequest('/api/tags');return true;}catch{}}}catch(e){console.warn('Ollama konnte nicht gestartet werden:',e.message)}return false;}
